...
with open("1_input.txt") as iV:
    for line in iV:
        fuelTotal += getFuelNeeded(int(line))

print("Ex. 1: ", fuelTotal)
...
def fuelMass(Module_Mass):
    ...
    newMass = Module_Mass
    while True:
        newMass = getFuelNeeded(newMass)
        if newMass <= 0:
            break
        fuelRequired += newMass
    return fuelRequired
...
with open("1_input.txt") as iV:
    for line in iV:
        moduleMass = getFuelNeeded(int(line))
        fuelTotal2 += moduleMass + recursiveFuel(moduleMass)

print("Ex. 2: ", fuelTotal2)
